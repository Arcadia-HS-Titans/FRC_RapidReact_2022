...
void loop(){
  ...
  if(!blocks){
    ...
    double xx = pixy.ccc.blocks[biggest].m_x / 319.0; //turns into a percent of the screen
    double yy = pixy.ccc.blocks[biggest].m_y / 199.0; //319 and 199 were, we found, the dimensions of the screen
    double basis_x = 0.5;
    double basis_y = 0.8;
    piOutput = String(xx);
